...
def base10_to_baseK(num, K):
    ...
    if num == 0:
        return [0]
    assert K > 0, "Cannot convert {num} to base {K}, as {K} <= 0".format(num=num, K=K)
    rem = num % K
    result = []
...
def baseK_to_base10(num, K):
    result = 0
    assert K > 0, "Cannot convert {num} to base 10, as {K} <= 0".format(num=num, K=K)
...
class Barcoder(object):
    ...
    def __init__(self, n_data_symbols, n_check_symbols, bits_per_symbol, bases_per_symbol, seed=42):
        ...
        np.random.seed(seed)
        # TODO: Assign alternative nomenclature to the variable names Barcodes and Unbarcodes.
